...
export function Marketplace({ district }: MarketplaceProps) {
  ...
  const bestMarket = cropPrices.reduce((best: MarketPrice | undefined, current: MarketPrice) => {
    return current.price_per_quintal > (best?.price_per_quintal || 0) ? current : best;
  }, cropPrices[0]);

  const fertilizers = inputPrices.filter((i: InputPrice) => i.input_type === 'fertilizer');
  const seeds = inputPrices.filter((i: InputPrice) => i.input_type === 'seed');
  const pesticides = inputPrices.filter((i: InputPrice) => i.input_type === 'pesticide');
